...
{
    public class OturumAcmaController : Sayfa
    {
        public async Task<ActionResult> Cek(Models.OturumAcmaModel modeli)
        {
            var nedir = await modeli.ayrintiliAraKos(this);
            return basariBildirimi("/OturumAcma?id=" + nedir.kodu);
        }
        public async Task<ActionResult> Index(string id)
        {
            try
            {
                ...
                string tanitim = await Genel.dokumKisaAciklamaKos(this, "OturumAcma");
                gorunumAyari("", "", "Ana Sayfa", "/", "/OturumAcma/", tanitim);
                if (!oturumAcildimi())
        ...
        public async Task<ActionResult> Kart(long id)
        {
            try
            {
                ...
                string tanitim = await Genel.dokumKisaAciklamaKos(this, "OturumAcma");
                gorunumAyari("Kartı", "Kartı", "Ana Sayfa", "/", "/OturumAcma/", tanitim);
                enumref_YetkiTuru yetkiTuru = yetkiTuruBelirle(id);
                if (await yetkiVarmiKos("OturumAcma", yetkiTuru))
                {
                    Models.OturumAcmaModel modeli = new Models.OturumAcmaModel();
        ...
        [HttpPost]
        public async Task<ActionResult> Kaydet(Models.OturumAcmaModel gelen)
        {
            try
            {
                if (!oturumAcildimi())
