...
{
    public class IlceController : Sayfa
    {
        public async Task<ActionResult> Cek(Models.IlceModel modeli)
        {
            var nedir = await modeli.ayrintiliAraKos(this);
            return basariBildirimi("/Ilce?id=" + nedir.kodu);
        }
        public async Task<ActionResult> Index(string id)
        {
            ...
            {
                string tanitim = "...";
                tanitim = await Genel.dokumKisaAciklamaKos(this, "Ilce");
                gorunumAyari("", "", "Ana Sayfa", "/", "/Ilce/", tanitim);
                if (!oturumAcildimi())
        ...
        public async Task<ActionResult> Kart(long id)
        {
            try
            {
                ...
                string tanitim = "....";
                tanitim = await Genel.dokumKisaAciklamaKos(this, "Ilce");
                gorunumAyari("İlçe Kartı", "İlçe Kartı", "Ana Sayfa", "/", "/Ilce/", tanitim);
                enumref_YetkiTuru yetkiTuru = yetkiTuruBelirle(id);
                if (await yetkiVarmiKos("Ilce", yetkiTuru))
                {
                    Models.IlceModel modeli = new Models.IlceModel();
        ...
        [HttpPost]
        public async Task<ActionResult> Kaydet(Models.IlceModel gelen)
        {
            try
            {
                if (!oturumAcildimi())
