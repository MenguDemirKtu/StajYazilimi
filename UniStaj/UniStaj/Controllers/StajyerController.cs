...
{
 public class StajyerController : Sayfa
    {
public async Task<ActionResult> Cek( Models.StajyerModel modeli)
                            {
                                var nedir = await modeli.ayrintiliAraKos(this);
                                return basariBildirimi("/Stajyer?id=" + nedir.kodu);
                            }
    public async Task<ActionResult> Index(string id)
        {
try{
            string tanitim = "...";
   tanitim = await Genel.dokumKisaAciklamaKos(this, "Stajyer"); 
 gorunumAyari("", "", "Ana Sayfa", "/", "/Stajyer/", tanitim); 
            if (!oturumAcildimi())
...
try{
            ...
            string tanitim = "....";
   tanitim = await Genel.dokumKisaAciklamaKos(this, "Stajyer"); 
 gorunumAyari("Stajyer Kartı", "Stajyer Kartı", "Ana Sayfa", "/", "/Stajyer/", tanitim); 
            enumref_YetkiTuru yetkiTuru = yetkiTuruBelirle(id);
if (await yetkiVarmiKos("Stajyer", yetkiTuru))
            {
  Models.StajyerModel modeli = new Models.StajyerModel(); 
...
Models.StajyerModel modeli = new Models.StajyerModel();
        ...
        [HttpPost]
   public async Task<ActionResult> Kaydet(Models.StajyerModel gelen)
        {
            try
            { 
               if (!oturumAcildimi())
