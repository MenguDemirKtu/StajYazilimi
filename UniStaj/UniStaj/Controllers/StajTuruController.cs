...
{
    public class StajTuruController : Sayfa
    {
        public async Task<ActionResult> Cek(Models.StajTuruModel modeli)
        {
            var nedir = await modeli.ayrintiliAraKos(this);
            return basariBildirimi("/StajTuru?id=" + nedir.kodu);
        }
        public async Task<ActionResult> Index(string id)
        {
            ...
            {
                string tanitim = "...";
                tanitim = await Genel.dokumKisaAciklamaKos(this, "StajTuru");
                gorunumAyari("", "", "Ana Sayfa", "/", "/StajTuru/", tanitim);
                if (!oturumAcildimi())
        ...
        public async Task<ActionResult> Kart(long id)
        {
            try
            {
                ...
                string tanitim = "....";
                tanitim = await Genel.dokumKisaAciklamaKos(this, "StajTuru");
                gorunumAyari("Staj Türü Kartı", "Staj Türü Kartı", "Ana Sayfa", "/", "/StajTuru/", tanitim);
                enumref_YetkiTuru yetkiTuru = yetkiTuruBelirle(id);
                if (await yetkiVarmiKos("StajTuru", yetkiTuru))
                {
                    Models.StajTuruModel modeli = new Models.StajTuruModel();
        ...
        [HttpPost]
        public async Task<ActionResult> Kaydet(Models.StajTuruModel gelen)
        {
            try
            {
                if (!oturumAcildimi())
