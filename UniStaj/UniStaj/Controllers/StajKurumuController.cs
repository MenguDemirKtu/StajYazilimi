...
{
    public class StajKurumuController : Sayfa
    {
        public async Task<ActionResult> Cek(Models.StajKurumuModel modeli)
        {
            var nedir = await modeli.ayrintiliAraKos(this);
            return basariBildirimi("/StajKurumu?id=" + nedir.kodu);
        }
        public async Task<ActionResult> Index(string id)
        {
            ...
            {
                string tanitim = "...";
                tanitim = await Genel.dokumKisaAciklamaKos(this, "StajKurumu");
                gorunumAyari("", "", "Ana Sayfa", "/", "/StajKurumu/", tanitim);
                if (!oturumAcildimi())
        ...
        public async Task<ActionResult> Kart(long id)
        {
            try
            {
                ...
                string tanitim = "....";
                tanitim = await Genel.dokumKisaAciklamaKos(this, "StajKurumu");
                gorunumAyari("Staj Kurumu Kartı", "Staj Kurumu Kartı", "Ana Sayfa", "/", "/StajKurumu/", tanitim);
                enumref_YetkiTuru yetkiTuru = yetkiTuruBelirle(id);
                if (await yetkiVarmiKos("StajKurumu", yetkiTuru))
                {
                    Models.StajKurumuModel modeli = new Models.StajKurumuModel();
        ...
        [HttpPost]
        public async Task<ActionResult> Kaydet(Models.StajKurumuModel gelen)
        {
            try
            {
                if (!oturumAcildimi())
